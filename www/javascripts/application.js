...
case"float":return"Float";default:throw"invalid parseType"}}function generateRegex(settings){if(settings.symbol===""){return new RegExp("[^\\d"+settings.decimalSymbol+"-]","g")
  ...
  window.App.Populate = Backbone.Model.extend({
    run: function() {
      this.populate(App.Budgets, this.budgetSeeds());
      this.populate(App.Incomes, this.incomeSeeds());
      this.populate(App.BudgetedExpenses, this.billSeeds());
      return this.populate(App.Savings, this.savingsSeeds());
    },
    populate: function(collectionClass, seeds) {
      var newRows;
      collectionClass.fetch();
      collectionClass.reset();
      return newRows = _.map(seeds, function(seed) {
        return collectionClass.create(seed);
      });
    },
    billSeeds: function() {
      return [
  ...
  window.App.BudgetedSingleView = App.SingleView.extend({
